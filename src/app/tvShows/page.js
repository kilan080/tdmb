...
export default function Page() {
  ...
  const fetchMovies = async () => {
    try{
      const res = await axios.get(`${config.baseUrl}${config.subUrl.tvshows}`, {
        headers: {
          ...
          return(
            <div className='tv-card' key={movie.id || index}>
              ...
              <div className='tv_info'>
                ...
                <p className='tv-date'>{movie.release_date || movie.first_air_date}</p>
                <p className='tv-detail'>{getFirstParagraph(movie.overview)}...</p>
              </div>
            </div>
          );
        })}
